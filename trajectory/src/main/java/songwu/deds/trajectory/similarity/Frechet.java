...
public class Frechet<U extends TrajectoryUnit<U>, T extends Trajectory<U>> implements SimilarityMeasure<U, T>{
    ...
    public Frechet(String innerDistanceName) {
    ...
    public double apply(T t1, T t2){
        ...
        if(len1 == 1){
            ...
            for(int pos = 0; pos < len2; pos++){
                answer = Math.max(answer, innerDistance(u, t2.getUnit(pos)));
            }
            return answer;
        }else {
            double[] row = new double[len1];
            u = t2.getUnit(0);
            row[0] = innerDistance(u, t1.getUnit(0));
            for(int pos = 1; pos < len1; pos++){
                row[pos] = Math.max(row[pos-1], innerDistance(u, t1.getUnit(pos)));
            }

            for(int outer = 1; outer < len2; outer++){
                u = t2.getUnit(outer);
                cell_previous = Math.max(innerDistance(u, t1.getUnit(0)), row[0]);

                for(int inner = 1; inner < len1; inner++){
                    cell_now = Math.max(innerDistance(u, t1.getUnit(inner)), Statistics.min(cell_previous, row[inner], row[inner-1]));
