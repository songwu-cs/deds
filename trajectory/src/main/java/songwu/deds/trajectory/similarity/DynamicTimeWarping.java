    ...
    public double apply(T t1, T t2){
        ...
        if(len1 == 1){
            ...
            for(int pos = 0; pos < len2; pos++){
                answer += u.distance(t2.getUnit(pos));
            }
            return answer;
        }else {
            ...
            for(int pos = 0; pos < len1; pos++){
                row[pos] = u.distance(t1.getUnit(pos));
            }
            ...
            for(int outer = 1; outer < len2; outer++){
                u = t2.getUnit(outer);
                cell_previous = u.distance(t1.getUnit(0)) + row[0];

                for(int inner = 1; inner < len1; inner++){
                    cell_now = u.distance(t1.getUnit(inner)) + Statistics.min(cell_previous, row[inner], row[inner-1]);
