...
public class CriticalTimeStampedPointTConvexFinerSpeed10Plus extends CriticalTimestampedPoint{
    private int gap;
    private int history;
    ...
    public CriticalTimeStampedPointTConvexFinerSpeed10Plus setGap(int gap) {
        this.gap = gap; return this;
    }

    public CriticalTimeStampedPointTConvexFinerSpeed10Plus setSpeedMin(double speed_min) {
        this.speed_min = speed_min; return this;
    }

    public CriticalTimeStampedPointTConvexFinerSpeed10Plus setSpeedSlowMotion(double speed_slow_motion) {
        this.speed_slow_motion = speed_slow_motion; return this;
    }

    public CriticalTimeStampedPointTConvexFinerSpeed10Plus setHistory(int history) {
        this.history = history; return this;
    }

    public CriticalTimeStampedPointTConvexFinerSpeed10Plus setSmoothThreshold(double smooth_threshold) {
        this.smooth_threshold = smooth_threshold; return this;
    }

    public CriticalTimeStampedPointTConvexFinerSpeed10Plus setSpeedAlpha(double speed_alpha) {
        ...
        while (true){
            ...
            for(int k = history + 1; k < len; k++){
                ...
                if(current.getTimeGap() > gap){
                    ...
                    if (flag10Plus){
                        critical.addPoint(new CriticalPoint().setType("speed10PlusEnd").copy(previous));
                        flag10Plus = false;
                    }
                    if(stop_flag){
                ...
                if(_s > 10 && (!flag10Plus)){
                    critical.addPoint(new CriticalPoint().setType("speed10PlusStart").copy(current));
                    flag10Plus = true;
                }else if((_s <= 10) && flag10Plus){
                    critical.addPoint(new CriticalPoint().setType("speed10PlusEnd").copy(current));
                    flag10Plus = false;
                }
            ...
            if(flag10Plus)
                critical.addPoint(new CriticalPoint().setType("speed10PlusEnd").copy(query.getUnit(query.size() - 1)));
    ...
    public List<CriticalPointInterval> point2interval(CriticalPointT ct){
        ...
        buffer.put("speed10PlusStart", new ArrayList<>());
        buffer.put("speed10PlusEnd", new ArrayList<>());
        ...
        List<CriticalPoint> speed10PlusStart = buffer.get("speed10PlusStart");
        List<CriticalPoint> speed10PlusEnd = buffer.get("speed10PlusEnd");
        ...
        counterInterval = 0;
        for(CriticalPoint cp : speed10PlusStart){
            if(speed10PlusEnd.size() > counterInterval){
                intervals.add(new CriticalPointInterval().setType("speed10Plus")
                        .setRank(counterInterval + "")
                        .setStartTime(cp.getTimestamp())
                        .setEndTime(speed10PlusEnd.get(counterInterval).getTimestamp())
                        .setId(ct.trajId()));
                counterInterval++;
            }else {
                System.out.println(ct.trajId() + " --> speed10Plus: [" + speed10PlusStart.size() + "," + speed10PlusEnd.size() + "]");
                break;
            }
        }
